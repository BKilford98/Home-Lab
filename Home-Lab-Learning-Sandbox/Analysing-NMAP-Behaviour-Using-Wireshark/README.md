...
- Kali Linux VM (Attacker) (Right)
- Labuntu VM (Victim) (Left)

## Preparations

Firstly, I will ensure that both devices are on the same subnet to ensure pfsense doesn’t intervene with our nmap scans.
Then I will install and start an SSH server on my Labuntu VM so we have an open port to scan with nmap.

![Installing SSH server on Labuntu VM](Screenshots/installing-openssh-server.png)

Next we will scan for the open port on our Kali VM using nmap's service scan.

![Finding our open port using nmap's service scan](Screenshots/finding-the-open-port.png)

We successfully found our SSH server on TCP port 22.

## Sniffing NMAP Packets With Wireshark
### Full TCP Scan
...
![Full TCP Scan](Screenshots/full-tcp-scan.png)
...
![Stealth Scan](Screenshots/syn-scan.png)
...
![ACK Scan](Screenshots/ack-scan.png)
...
![Null Scan Open Port](Screenshots/null-scan-open-port.png)
...
Finally we will look at a FIN Scan

**Open Port (22)**

![FIN Scan Open Port](Screenshots/fin-scan-open-port.png)

**Closed Port (60)**

![FIN Scan Closed Port](Screenshots/fin-scan-closed-port.png)
